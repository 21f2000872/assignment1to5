...
# Pipeline sessions intentionally own independent revision, event, and cache
# state. The deployment instructions use one Uvicorn process, so the state is
# shared by every request handled by that process.
PIPELINE_SESSIONS: dict[str, dict[str, Any]] = {}
PIPELINE_SESSIONS_LOCK = asyncio.Lock()
...
# ---------------------------------------------------------------------------
# Assignment 6: content-addressed pipeline recovery controller
# ---------------------------------------------------------------------------
...
def pipeline_error(code: str) -> JSONResponse:
    return JSONResponse({"error": code}, status_code=409)


def pipeline_request_valid(body: Any) -> bool:
    ...
    if not is_utf8_string(session) or session == "":
        return False
    if not is_safe_integer(revision, minimum=1):
        return False
    if not isinstance(inputs, dict) or not isinstance(events, list):
        return False
    return all(
        is_utf8_string(inputs.get(name)) and inputs[name] != ""
        for name in PIPELINE_REQUIRED_INPUTS
    )


def pipeline_inputs_fingerprint(inputs: dict[str, Any]) -> str:
...
def new_pipeline_session(
    revision: int, inputs: dict[str, Any], fingerprint: str
) -> dict[str, Any]:
    return {
        "revision": revision,
        "inputs": copy.deepcopy(inputs),
        "inputsFingerprint": fingerprint,
        "caches": {node: {} for node in PIPELINE_NODES},
        "nodeStates": {},
        "eventIds": {},
    }


def pipeline_cache_key(values: list[str]) -> str:
    return hashlib.sha256(compact_json(values).encode("utf-8")).hexdigest()


def pipeline_dependency_values(
    node: str,
    inputs: dict[str, Any],
    ...
    if node == "train":
        return {
            "prepareArtifact": parent_artifact,
    ...
    if node == "register":
        return {
            "evaluateArtifact": parent_artifact,
            "schemaDigest": inputs["schemaDigest"],
        }
    return {
        "registerArtifact": parent_artifact,
        "publishConfig": inputs["publishConfig"],
    }


def build_pipeline_contexts(state: dict[str, Any]) -> list[dict[str, Any]]:
    contexts: list[dict[str, Any]] = []
    inputs = state["inputs"]

    for index, node in enumerate(PIPELINE_NODES):
        parent = contexts[index - 1] if index else None
        parent_reusable = parent is None or parent["reusable"]
        parent_artifact = (
            parent["cacheEntry"]["artifactDigest"]
            if parent is not None and parent["reusable"]
            else None
        )
        dependencies = pipeline_dependency_values(node, inputs, parent_artifact)
        cache_key = (
            pipeline_cache_key(list(dependencies.values()))
            if parent_reusable
            else None
        )
        cache_entry = (
            state["caches"][node].get(cache_key)
        ...
        reusable = cache_entry is not None
        own_terminal = (
            parent_reusable
            and node_state is not None
            and node_state["status"] == "terminal_failed"
        )
        terminal_chain = (
            own_terminal
            or (
                parent is not None
                and not parent_reusable
                and parent["terminalChain"]
            )
        )
        dependency_output = dict(dependencies)
        dependency_output["cacheKey"] = cache_key
        contexts.append(
            {
                "node": node,
                "parentReusable": parent_reusable,
                "dependencies": dependency_output,
                "cacheKey": cache_key,
                "cacheEntry": cache_entry,
                "nodeState": node_state,
                "reusable": reusable,
                "terminalChain": terminal_chain,
            }
        )
...
def canonical_pipeline_event(event: dict[str, Any]) -> str:
    ordered_event = {field: event[field] for field in PIPELINE_EVENT_FIELDS}
    return json.dumps(
        ordered_event,
        ensure_ascii=False,
        separators=(",", ":"),
...
def pipeline_event_structure_valid(event: Any) -> bool:
    return (
        isinstance(event, dict)
        and set(event) == set(PIPELINE_EVENT_FIELDS)
        and is_utf8_string(event.get("eventId"))
        and event["eventId"] != ""
    )


def pipeline_event_semantics_valid(
    event: dict[str, Any], node: str, cache_key: str
) -> bool:
    ...
    if not is_safe_integer(attempt, minimum=1):
        return False
    if not isinstance(status, str) or status not in PIPELINE_STATUSES:
        return False
    if status == "succeeded":
        if not is_utf8_string(artifact) or artifact == "":
            return False
    elif artifact is not None:
        return False

    if status == "succeeded" and node in {"register", "publish"}:
        return receipt == f"receipt:{node}:{cache_key}"
    return receipt is None


def accept_pipeline_event(
    state: dict[str, Any],
    event: dict[str, Any],
    canonical_event: str,
    ...
    status = event["status"]
    attempt = event["attempt"]
    state["eventIds"][event_id] = canonical_event

    if status == "succeeded":
        state["caches"][node][context["cacheKey"]] = {
            "artifactDigest": event["artifactDigest"],
            "eventId": event_id,
        }
        state["nodeStates"].pop(node, None)
        return
...
def process_pipeline_events(
    ...
    for event in events:
        if not pipeline_event_structure_valid(event):
            return "INVALID_EVENT", [], []

        event_id = event["eventId"]
        canonical_event = canonical_pipeline_event(event)
        prior_event = state["eventIds"].get(event_id)
        if prior_event is not None:
            if prior_event == canonical_event:
        ...
        if (
            not is_safe_integer(event_revision, minimum=1)
            or event_revision != state["revision"]
        ):
            ignored.append(event_id)
            continue
        ...
        contexts = build_pipeline_contexts(state)
        context = contexts[PIPELINE_NODES.index(node)]
        if not context["parentReusable"] or context["cacheKey"] is None:
            ignored.append(event_id)
            continue
        if event["key"] != context["cacheKey"]:
            ignored.append(event_id)
            continue
        if not pipeline_event_semantics_valid(event, node, context["cacheKey"]):
            ignored.append(event_id)
            continue

        cache_entry = context["cacheEntry"]
        node_state = context["nodeState"]
        status = event["status"]
        attempt = event["attempt"]
        ...
        if cache_entry is not None:
            if (
                status == "succeeded"
                and event["artifactDigest"] != cache_entry["artifactDigest"]
            ):
        ...
        if node_state is None:
            if status == "started" and attempt == 1:
                accept_pipeline_event(state, event, canonical_event, context)
                accepted.append(event_id)
            else:
                ignored.append(event_id)
            continue

        previous_status = node_state["status"]
        previous_attempt = node_state["attempt"]
        ...
        if previous_status == "started":
            if (
                ...
                and status in PIPELINE_COMPLETION_STATUSES
            ):
                accept_pipeline_event(state, event, canonical_event, context)
                accepted.append(event_id)
                continue
        ...
        if previous_status == "retryable_failed":
            if status == "started" and attempt == previous_attempt + 1:
                accept_pipeline_event(state, event, canonical_event, context)
                accepted.append(event_id)
                continue
            return "STATUS_CONFLICT", [], []
    ...
    return None, accepted, ignored


def make_pipeline_response(
    state: dict[str, Any], accepted: list[str], ignored: list[str]
) -> dict[str, Any]:
    ...
    for context in contexts:
        ...
        if cache_entry is not None:
            action = "reuse"
            reason = "CACHE_HIT"
            triggering_ids = [cache_entry["eventId"]]
        elif not context["parentReusable"]:
            action = "block"
            reason = (
                "UPSTREAM_TERMINAL"
                if context["terminalChain"]
            ...
            action = "block"
            reason = "RUNNING"
            triggering_ids = [node_state["eventId"]]
        elif node_state["status"] == "retryable_failed":
            action = "rerun"
        ...
        nodes.append(
            {
                ...
                "action": action,
                "reasonCodes": [reason],
                "dependencyDigests": context["dependencies"],
...
async def handle_pipeline_request(body: dict[str, Any]) -> JSONResponse:
    session = body["session"]
    revision = body["revision"]
    inputs = body["inputs"]
    fingerprint = pipeline_inputs_fingerprint(inputs)

    async with PIPELINE_SESSIONS_LOCK:
        stored = PIPELINE_SESSIONS.get(session)
        if stored is None:
            working = new_pipeline_session(revision, inputs, fingerprint)
        else:
            working = copy.deepcopy(stored)
            if revision < working["revision"]:
                return pipeline_error("REVISION_CONFLICT")
            if revision == working["revision"]:
                if fingerprint != working["inputsFingerprint"]:
                    return pipeline_error("REVISION_CONFLICT")
            else:
                working["revision"] = revision
                working["inputs"] = copy.deepcopy(inputs)
                working["inputsFingerprint"] = fingerprint
                working["nodeStates"] = {}

        conflict, accepted, ignored = process_pipeline_events(
            working, body["events"]
        )
        if conflict is not None:
            return pipeline_error(conflict)

        PIPELINE_SESSIONS[session] = working
        response = make_pipeline_response(working, accepted, ignored)
...
async def pipeline(request: Request) -> JSONResponse:
    try:
        ...
        return pipeline_error("INVALID_REQUEST")

    if not pipeline_request_valid(body):
        return pipeline_error("INVALID_REQUEST")
    return await handle_pipeline_request(body)


# ---------------------------------------------------------------------------
# Assignment 7: verifiable UTF-8 model bundle and model-card gate
# ---------------------------------------------------------------------------
...
BUNDLE_INVENTORY_FILES = frozenset(BUNDLE_REQUIRED_FILES)
BUNDLE_UNSAFE_EXTENSIONS = (".bin", ".pt", ".pth", ".pkl", ".pickle")
...
def parse_bundle_json(text: str) -> tuple[bool, Any]:
    try:
    ...
    except (DuplicateJSONKey, json.JSONDecodeError, ValueError, TypeError):
        return False, None


def bundle_policy_valid(policy: Any) -> bool:
    if not isinstance(policy, dict):
        return False
    required_slices = policy.get("requiredSlices")
    if (
        not isinstance(required_slices, list)
        or not required_slices
        or not all(
            is_utf8_string(name) and name != "" for name in required_slices
        )
        or len(set(required_slices)) != len(required_slices)
    ):
        return False
    return all(
        is_utf8_string(policy.get(name)) and policy[name] != ""
        for name in ("license", "intendedUse", "limitations")
    )


def safe_bundle_filename(filename: Any) -> str:
    return filename if is_utf8_string(filename) else "<invalid>"


def recompute_bundle_inventory(
    files: dict[str, Any],
) -> tuple[bool, list[dict[str, Any]], Optional[str]]:
    inventory: list[dict[str, Any]] = []
    inventory_sources: list[tuple[str, str]] = []
    for filename, content in files.items():
        if filename == "inventory.json":
            continue
        if (
            not is_utf8_string(filename)
            or filename == ""
            or not is_utf8_string(content)
        ):
            return False, [], None
        inventory_sources.append((filename, content))

    inventory_sources.sort(key=lambda item: item[0].encode("utf-8"))
    for filename, content in inventory_sources:
        exact_bytes = content.encode("utf-8")
        inventory.append(
            {
                "name": filename,
    ...
    return True, inventory, inventory_digest


def valid_adapter_config(value: Any) -> bool:
    if not isinstance(value, dict):
        return False
    rank = value.get("r")
    targets = value.get("target_modules")
    return (
        is_safe_integer(rank, minimum=1)
        and isinstance(targets, list)
        and bool(targets)
        and all(is_utf8_string(target) and target != "" for target in targets)
        and len(set(targets)) == len(targets)
    )


def find_model_card_payloads(readme: str) -> list[Optional[str]]:
    payloads: list[Optional[str]] = []
    cursor = 0
    while True:
        start = readme.find(MODEL_CARD_PREFIX, cursor)
        if start < 0:
            break
        payload_start = start + len(MODEL_CARD_PREFIX)
        end = readme.find(MODEL_CARD_SUFFIX, payload_start)
        if end < 0:
            payloads.append(None)
            break
        payloads.append(readme[payload_start:end])
        cursor = end + len(MODEL_CARD_SUFFIX)
    return payloads


def verify_bundle_response(body: dict[str, Any]) -> dict[str, Any]:
    policy = body["policy"]
    files = body["files"]
    violations: set[str] = set()

    policy_valid = bundle_policy_valid(policy)
    if not policy_valid:
        violations.add("INVALID_POLICY")

    for required_name in BUNDLE_REQUIRED_FILES:
        if required_name not in files:
            violations.add(f"MISSING_FILE:{required_name}")

    for supplied_name, content in files.items():
        output_name = safe_bundle_filename(supplied_name)
        filename_valid = is_utf8_string(supplied_name) and supplied_name != ""
        if not filename_valid or not is_utf8_string(content):
            violations.add(f"INVALID_FILE:{output_name}")
        if supplied_name not in BUNDLE_INVENTORY_FILES:
            violations.add("UNTRACKED_FILE")
        if (
            is_utf8_string(supplied_name)
            and supplied_name.lower().endswith(BUNDLE_UNSAFE_EXTENSIONS)
        ):
            violations.add("UNSAFE_WEIGHTS")

    inventory_valid, recomputed_inventory, inventory_digest = (
        recompute_bundle_inventory(files)
    )
    supplied_inventory = files.get("inventory.json")
    if "inventory.json" in files:
        inventory_matches = False
        if is_utf8_string(supplied_inventory):
            inventory_json_valid, _ = parse_bundle_json(supplied_inventory)
            if not inventory_json_valid:
                violations.add("INVALID_JSON:inventory.json")
            if inventory_valid:
                inventory_matches = supplied_inventory == compact_json(
    ...
    adapter_config_text = files.get("adapter_config.json")
    if is_utf8_string(adapter_config_text):
        adapter_json_valid, adapter_config = parse_bundle_json(adapter_config_text)
        if not adapter_json_valid:
            violations.add("INVALID_JSON:adapter_config.json")
            adapter_config = None
        elif not valid_adapter_config(adapter_config):
            violations.add("INVALID_ADAPTER_CONFIG")

    model_artifact_digest: Optional[str] = None
    adapter_model_text = files.get("adapter_model.safetensors")
    if is_utf8_string(adapter_model_text):
        model_artifact_digest = hashlib.sha256(
            adapter_model_text.encode("utf-8")
        ).hexdigest()

    evaluation_file_digest: Optional[str] = None
    evaluation_text = files.get("evaluation.json")
    if is_utf8_string(evaluation_text):
        evaluation_file_digest = hashlib.sha256(
            evaluation_text.encode("utf-8")
        ).hexdigest()

    manifest: Any = None
    manifest_text = files.get("training_manifest.json")
    if is_utf8_string(manifest_text):
        manifest_json_valid, manifest = parse_bundle_json(manifest_text)
        if not manifest_json_valid:
            ...
            for field in BUNDLE_MANIFEST_FIELDS:
                if field not in manifest:
                    violations.add(f"MISSING_MANIFEST_FIELD:{field}")

            base_revision = manifest.get("baseRevision")
            if "baseRevision" in manifest and (
                not isinstance(base_revision, str)
                or HEX40_RE.fullmatch(base_revision) is None
            ):
                violations.add("MUTABLE_BASE_REVISION")

            invalid_nonempty_field = any(
                field in manifest
                and (
                    not is_utf8_string(manifest[field])
                    or manifest[field] == ""
                )
                for field in BUNDLE_MANIFEST_FIELDS
                if field != "baseRevision"
            )
            if invalid_nonempty_field:
                violations.add("INVALID_TRAINING_MANIFEST")

            claimed_model_digest = manifest.get("modelArtifactDigest")
            if (
                model_artifact_digest is not None
                and is_utf8_string(claimed_model_digest)
                and claimed_model_digest != ""
                and claimed_model_digest != model_artifact_digest
            ):
                violations.add("MODEL_ARTIFACT_MISMATCH")

            claimed_evaluation_digest = manifest.get(
                "evaluationArtifactDigest"
            )
            if (
                evaluation_file_digest is not None
                and is_utf8_string(claimed_evaluation_digest)
                and claimed_evaluation_digest != ""
                and claimed_evaluation_digest != evaluation_file_digest
            ):
                violations.add("EVALUATION_DIGEST_MISMATCH")

    evaluation: Any = None
    if is_utf8_string(evaluation_text):
        evaluation_json_valid, evaluation = parse_bundle_json(evaluation_text)
        if not evaluation_json_valid:
            violations.add("INVALID_JSON:evaluation.json")
            evaluation = None
        elif not isinstance(evaluation, dict):
            violations.add("INVALID_EVALUATION")
        else:
            bound_model_digest = evaluation.get("modelArtifactDigest")
            if not is_utf8_string(bound_model_digest) or bound_model_digest == "":
                violations.add("INVALID_EVALUATION")
            elif (
                model_artifact_digest is not None
                and bound_model_digest != model_artifact_digest
            ):
                violations.add("EVALUATION_ARTIFACT_MISMATCH")

            if not is_unit_floor(evaluation.get("aggregate")):
            ...
            if policy_valid:
                for slice_name in policy["requiredSlices"]:
                    ...
                    elif not is_unit_floor(slices[slice_name]):
                        violations.add(f"SLICE_RANGE:{slice_name}")

    readme = files.get("README.md")
    payloads = find_model_card_payloads(readme) if is_utf8_string(readme) else []
    if not payloads:
        ...
        payload = payloads[0]
        card_valid = payload is not None
        card: Any = None
        if card_valid:
            card_valid, card = parse_bundle_json(payload)
            card_valid = card_valid and isinstance(card, dict)
        if not card_valid:
            ...
            if any(
                name not in card or card[name] != expected
                for name, expected in expected_claims.items()
            ):
                violations.add("MODEL_CARD_MISMATCH")

    sorted_violations = sorted_codes(violations)
    return {
...
async def verify_bundle(request: Request) -> JSONResponse:
    ...
    if (
        ...
        return JSONResponse({"error": "INVALID_INPUT"}, status_code=400)
    return JSONResponse(verify_bundle_response(body))
