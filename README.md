# Assignment API Service: Assignments 1–7

This project implements all seven assignment endpoints:
...
- `test_pipeline.py` — tests pipeline caching, retries, receipts, rollback, and isolation
- `test_verify_bundle.py` — tests bundle inventory, lineage, evaluation, and model cards
...
6. In a second terminal, run the test:
   ...
   python test_verify_bundle.py http://127.0.0.1:8000
   ```

## Deploy on Render

1. Upload all ten files listed above to the root of a GitHub repository.
...
6. Test it, replacing the example URL with yours:
   ...
   python test_pipeline.py https://your-service-name.onrender.com
   python test_verify_bundle.py https://your-service-name.onrender.com
   ```

7. Submit only the base URL, without an endpoint path.

The graders themselves add `/build-corpus`, `/bqml`, `/promote`, `/adapt`, `/quantize`, `/pipeline`, or `/verify-bundle` and send POST requests.
